  ...
  return (
    <div
      className={`month-calendar${
        compact ? ' month-calendar--compact' : ''
      }`}
    >
      ...
      >
        ...
        {calendarDays.map((date) => {
          ...
          const isBooked = bookedDates.includes(value)
          const isAvailable = availableDates.includes(value)

          const isSelected =
            selectionMode === 'single' && value === selectedDate

          const isRangeStart =
            selectionMode === 'range' && value === rangeStart

          const isRangeEnd =
            selectionMode === 'range' && value === rangeEnd
          ...
          const classNames = [
            ...
            isSelected
              ...
              : '',
              isAvailable ? 'month-calendar__day--available' : '',
            isRangeStart
              ? 'month-calendar__day--range-start'
              : '',
            isRangeEnd
              ? 'month-calendar__day--range-end'
              : '',
            isInRange
              ? 'month-calendar__day--in-range'
              : '',
            isBooked
        ...
        {selectionMode === 'range' &&
          (!rangeStart ? (
            ...
            <>
              Período selecionado:{' '}
              <strong>
                {formatFullDate(rangeStart)} a{' '}
                {formatFullDate(rangeEnd)}
              </strong>
            </>
          ) : (
            <>
              Início selecionado:{' '}
              <strong>{formatFullDate(rangeStart)}</strong>.
